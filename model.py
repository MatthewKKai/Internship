...
date = np.load('cixiangliang_res.npy')
print(date.shape)
...
y = y.ravel()
print(y.shape)
X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=0.3, random_state=32)  # 随机阈值是4
print('------')
from sklearn.model_selection import KFold
...
def DNN_base_v1(X_train, y_train):
    ...
    model.fit(X_train, y_train, epochs=3600, batch_size=100, validation_split=0.2, verbose=0, shuffle=True)
...
def caculate_main_p(pre_labels, target):
    ...
    Recall = recall_score(target_labels_reg, pre_labels_reg)
    Precision = precision_score(target_labels_reg, pre_labels_reg)
    F1 = f1_score(target_labels_reg, pre_labels_reg)
...
cc.fit_stacked_model(meta_model=nb)
...
plt.plot(fpr, tpr, color='blueviolet',
...
dd.fit_stacked_model(meta_model=gbm_model)
...
ee.fit_stacked_model(meta_model=xgb_model)
...
#6
ff = StackingAverageModels_build2(X_train, y_train, X_test, y_test)
ff.load_all_models(doc_dir=r'./tmp_models', n_models=5)
ff.fit_stacked_model(meta_model = rdf)
y_pred = ff.stacked_prediction()
